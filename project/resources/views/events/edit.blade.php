...
<div class="max-w-4xl mx-auto py-10">
    ...
    <form action="{{ route('events.update', ['evenement' => $evenement->id]) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow-lg space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="shortDescription" value="{{ old('shortDescription', $evenement->shortDescription) }}" required maxlength="150"
        ...
        <div>
            ...
            <textarea name="description" rows="4" required
                ...
                placeholder="Description complète de votre événement...">{{ old('description', $evenement->description) }}</textarea>
        ...
        <div>
            ...
            @if($evenement->image_url)
                <img src="{{ $evenement->image_url }}" alt="Image actuelle" class="mt-2 w-48 rounded">
            @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', $evenement->date->format('Y-m-d')) }}" required
            ...
            <div>
                ...
                <input type="time" name="time" value="{{ old('time', $evenement->date->format('H:i')) }}" required
        ...
        <div>
            ...
            <input type="text" name="lieu" value="{{ old('lieu', $evenement->lieu) }}" required
        ...
        <div>
            ...
            <select name="category" required
                ...
                @foreach($categories as $cat)
                    <option value="{{ $cat }}" {{ old('category', $evenement->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <div class="flex space-x-4">
                <label class="flex items-center space-x-2">
                    <input type="radio" name="type" value="gratuit" {{ old('type', $evenement->type) === 'gratuit' ? 'checked' : '' }} required>
                    <span>Gratuit</span>
                </label>
                <label class="flex items-center space-x-2">
                    <input type="radio" name="type" value="payant" {{ old('type', $evenement->type) === 'payant' ? 'checked' : '' }} required>
                    <span>Payant</span>
                </label>
            </div>
        ...
        {{-- Prix si payant --}}
        <div id="priceField" class="{{ old('type', $evenement->type) === 'payant' ? '' : 'hidden' }}">
            <label class="block text-sm font-medium text-gray-700 mb-1">Prix du billet (EUR) *</label>
            <input type="number" step="0.01" min="0" name="tarif" value="{{ old('tarif', $evenement->tarif) }}"
        ...
        <div>
            ...
            <input type="number" name="places_max" min="1" value="{{ old('places_max', $evenement->places_max) }}" required
        ...
        <div class="flex justify-end">
            <button type="submit"
                class="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-medium">Mettre à jour l'événement</button>
        </div>
    </form>
</div>

<script>
    // Basculer l'affichage du prix
